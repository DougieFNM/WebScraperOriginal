...
(async () => {
    ...
    const FUNOPTS = ["--images", "--importify", "--scrape-media", "--patio"];
    ...
    if (pargs.length > 0 && FUNOPTS.includes(pargs[0])) {
        ...
        } else if (FUNOPTS[3] === pargs[0]) {
            console.log("I want to filter the product for patio furniture "+scrapeBrand);
            await Gmg_Async.ss(3);
            await scraperClass.patioSortingFunction();
        }
    } else {
